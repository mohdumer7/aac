...
const page = () => {
    ...
    const fieldsToAdd = [
        ...
        { fieldName: 'printerName', path: ['printer', 'name'] },

    ];

    const transformedData: any = transformData(printerUsageData?.data, fieldsToAdd);
console.log('printerUsageData', transformedData);
    ...
    transformedData?.forEach(entry => {
        const printer = entry?.printerName;
        ...
        if (targetGroup) {
            ...
            if (!targetGroup[printer]) targetGroup[printer] = {};
            if (!targetGroup[printer][key]) {
                targetGroup[printer][key] = {
            ...
            targetGroup[printer][key].bwCopy += bwCopy;
            targetGroup[printer][key].colorCopy += colorCopy;
            targetGroup[printer][key].bwPrint += bwPrint;
            targetGroup[printer][key].colorPrint += colorPrint;
        }
    });
    ...
    Object.entries(groupedCurrent).forEach(([dept, employees]: [string, any]) => {
        ...
        netBw += deptNetBwCopy + deptNetBwPrint;
        netColor += deptNetColorCopy + deptNetColorPrint;


        // Insert department total row
        formattedData.push({
            serial: serial++, // Or something like 'Total'
            printer: dept,
            printerName: dept,
            id: "",
            userName: "",
            ...
            // optional: helps if you want to style it differently
        });
    });

    formattedData.push({
        serial: '', // leave blank or use 'Total'
        printer: 'Total',
        printerName: 'Total',
        id: "",
        userName: "",
        netCopyBw: '', // optional: leave blank or compute separately if needed
        netCopyColor: '',
        netPrintBw: '',
        netPrintColor: '',
        bw: netBw,
        color: netColor,
        total: netBw + netColor,
        isTotalRow: true // optional: for separate styling
    });
    ...
    const exportPrinterReportToPDF = (data, selectedMonth, selectedYear) => {
        ...
        const doc = new jsPDF();
        const title = `Printer Wise Summary Report Of B&W - Color Printing For ${month} ${selectedYear}`;
        ...
        autoTable(doc, {
            startY: 20,
            head: [['SN#', 'Printer', 'B&W', 'Color', 'Total']],
            body: rows,
            columnStyles: {
                4: { halign: 'right' }, // B&W
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ({ row }: { row: any }) => <div >{row.getValue("printerName")}</div>,
        },
    ...
    const onchangeData = async ({ id, fieldName, name, parentId, position, email, phone, location }: { id: any; fieldName: string; name: string; parentId?: string; position?: string; email?: string; phone?: string; location?: string }) => {
